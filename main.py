...
def find_carrier_frequency(samples, fs, freq_range=(300, 3000)):
    ...
    if len(idx) == 0:
        raise ValueError("Частота не найдена в заданном диапазоне")
...
def find_best_shift_and_duration(samples, fs, freq, min_dur=0.04, max_dur=0.135, step=0.005):
    ...
    for dur in np.arange(min_dur, max_dur + step, step):
        ...
        for shift in shifts:
            ...
            threshold = (np.max(corrs) + np.min(corrs)) / 2
            corrs_high = [c for c in corrs if c > threshold]
            corrs_low = [c for c in corrs if c <= threshold]
            if len(corrs_high) == 0 or len(corrs_low) == 0:
                sep = 0
            else:
                sep = (np.mean(corrs_high) - np.mean(corrs_low)) / (np.std(corrs_high) + np.std(corrs_low) + 1e-6)
            if sep > best_sep:
                best_sep = sep
                best_dur = dur
                best_shift = shift
...
def main():
    ...
    n = len(sorted_corrs)
    threshold = (sorted_corrs[n // 2] + sorted_corrs[n // 2 + 1]) / 2
    bits = ''.join(['1' if c > threshold else '0' for c in corrs])
    text = bits_to_ascii(bits)

    # Вывод корреляций по сегментам
